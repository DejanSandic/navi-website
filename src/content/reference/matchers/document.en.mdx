...
Because matchers are just generators, it's possible to create your own matchers. As an example, it would be possible to create a `withTimeout()` function that returns a matcher that emits an error if its child matchers take too much time to resolve.
